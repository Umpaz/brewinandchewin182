...
public class KegRecipe implements Recipe<RecipeWrapper>
{
	...
	public KegRecipe(ResourceLocation id, String group, NonNullList<Ingredient> inputItems, ItemStack output, ItemStack container, float experience, int cookTime) {
		this.id = id;
		this.group = group;
	...
	private static class Serializer extends ForgeRegistryEntry<RecipeSerializer<?>> implements RecipeSerializer<KegRecipe>
	{
		...
		public KegRecipe fromJson(ResourceLocation recipeId, JsonObject json) {
			...
			if (inputItemsIn.isEmpty()) {
				...
				return new KegRecipe(recipeId, groupIn, inputItemsIn, outputIn, container, experienceIn, cookTimeIn);
			}
		}
